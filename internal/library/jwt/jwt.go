...
func CreateJwtGoToken(audience string, id string) (string, error) {
	expiresTime := time.Now().Unix() + gconv.Int64(config.Cfg.Login.UserTime)
	...
	var jwtSecret = []byte(gconv.String(config.Cfg.Login.Secret))
...
func ParseJwtGoToken(token string) (*jwt.StandardClaims, error) {
	jwtToken, err := jwt.ParseWithClaims(token, &jwt.StandardClaims{}, func(token *jwt.Token) (i interface{}, e error) {
		return []byte(gconv.String(config.Cfg.Login.Secret)), nil
	})
