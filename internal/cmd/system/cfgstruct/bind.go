...
func UseDevEnvOrDefaults(identify string) BindOpt {
	dev := true
	test := false
	return BindOpt{isDev: &dev, isTest: &test, identify: identify}
}
...
func UseReleaseEnvOrDefaults(identify string) BindOpt {
	dev := false
	test := false
	return BindOpt{isDev: &dev, isTest: &test, identify: identify}
}
...
func UseTestEnvOrDefaults(identify string) BindOpt {
	dev := false
	test := true
	return BindOpt{isDev: &dev, isTest: &test, identify: identify}
}
...
// 'common_config'. This works by traversing the common_config struct using the 'reflect'
// package.
...
func bind(flags FlagSet, config interface{}, opts ...BindOpt) {
	...
	if ptrType.Kind() != reflect.Ptr {
		panic(fmt.Sprintf("invalid common_config type: %#v. Expecting pointer to struct.", config))
	}
	isDev := true
	isTest := false
	identify := "Env"
	...
	bindNewConfig(flags, "", reflect.ValueOf(config).Elem(), vars, setupCommand, false, isDev, isTest, identify)
}
...
func bindConfig(flags FlagSet, prefix string, val reflect.Value, vars map[string]confVar, setupCommand, setupStruct bool, isDev, isTest bool) {
	if val.Kind() != reflect.Struct {
		panic(fmt.Sprintf("invalid common_config type: %#v. Expecting struct.", val.Interface()))
	}
	typ := val.Type()
...
func bindNewConfig(flags FlagSet, prefix string, val reflect.Value, vars map[string]confVar, setupCommand, setupStruct bool, isDev, isTest bool, identify string) {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		if fieldValue, ok := fieldAddr.(pflag.Value); ok {
			help := field.Tag.Get("help")
			def := getNewDefault(field.Tag, isTest, isDev, flagName, identify)
			if field.Tag.Get("internal") == "true" {
				if def != "" {
					panic(fmt.Sprintf("unapplicable default value set for internal flag: %s", flagName))
				}
		...
		case reflect.Struct:
			if field.Anonymous {
				bindNewConfig(flags, prefix, fieldVal, vars, setupCommand, onlyForSetup, isDev, isTest, identify)
			} else {
				bindNewConfig(flags, flagName+".", fieldVal, vars, setupCommand, onlyForSetup, isDev, isTest, identify)
			}
		case reflect.Array:
			digits := len(fmt.Sprint(fieldVal.Len()))
			for j := 0; j < fieldVal.Len(); j++ {
				padding := strings.Repeat("0", digits-len(fmt.Sprint(j)))
				bindNewConfig(flags, fmt.Sprintf("%s.%s%d.", flagName, padding, j), fieldVal.Index(j), vars, setupCommand, onlyForSetup, isDev, isTest, identify)
			}
		default:
			help := field.Tag.Get("help")
			def := getNewDefault(field.Tag, isTest, isDev, flagName, identify)
...
func getDefault(tag reflect.StructTag, isTest, isDev bool, flagname string) string {
	...
	if isTest {
		order = []string{"testEnv", "devEnv", "testDefault", "devDefault", "default"}
		opposites = []string{"releaseEnv", "releaseDefault"}
	} else if isDev {
		order = []string{"devEnv", "devDefault", "default"}
		opposites = []string{"releaseEnv", "testEnv", "releaseDefault", "testDefault"}
	} else {
		order = []string{"releaseEnv", "releaseDefault", "default"}
		opposites = []string{"devEnv", "testEnv", "devDefault", "testDefault"}
	}
...
func getNewDefault(tag reflect.StructTag, isTest, isDev bool, flagname string, identify string) string {
	var order []string
	var opposites []string
	if isTest {
		identify = fmt.Sprintf("test%s", identify)
		order = []string{"testEnv", "testDefault", "devEnv", "devDefault", "default"}
		opposites = []string{"releaseEnv", "releaseDefault"}
	} else if isDev {
		identify = fmt.Sprintf("dev%s", identify)
		order = []string{"devEnv", "devDefault", "default"}
		opposites = []string{"releaseEnv", "testEnv", "releaseDefault", "testDefault"}
	} else {
		identify = fmt.Sprintf("release%s", identify)
		order = []string{"releaseEnv", "releaseDefault", "default"}
		opposites = []string{"devEnv", "testEnv", "devDefault", "testDefault"}
	}
...
// FindConfigFileParam returns '--common_config' param from os.Args (if exists).
func FindConfigFileParam() string {
	return FindFlagEarly("common_config")
}
...
func FindEnvsParam() string {
	return FindFlagEarly("env")
	//return FindFlagEarlyFormat("env", format, formatN)
}
...
func FindFlagEarlyFormat(flagName string, mats string, matsN string) string {
	format := "--%s="
	formatN := "--%s"
	if mats != "" {
		format = mats
	...
	for i, arg := range os.Args {
		if strings.HasPrefix(arg, fmt.Sprintf(format, flagName)) {
			return strings.TrimPrefix(arg, fmt.Sprintf(format, flagName))
		} else if arg == fmt.Sprintf(formatN, flagName) && i < len(os.Args)-1 {
			return os.Args[i+1]
		}
	}
	return ""
...
// SetupFlag sets up flags that are needed before `flag.Parse` has been called.
func SetupEnvFlag(cmd *cobra.Command, dest *string, name, short, value, usage string) {
	if foundValue := FindFlagEarlyFormat(name, format, formatN); foundValue != "" {
		value = foundValue
	}
	cmd.PersistentFlags().StringVarP(dest, name, short, value, usage)
	cmd.PersistentFlags().Lookup("name").NoOptDefVal = value
...
func EnvsType() string {
	// define a flag so that the flag parsing system will be happy.
	env := strings.ToLower(FindEnvsParam())
	if env != "" {
		return env
	}
	return Defaults
...
// DefaultsFlag sets up the defaults=dev/release flag options, which is needed
// before `flag.Parse` has been called.
func EnvsFlag(cmd *cobra.Command) BindOpt {
	// define a flag so that the flag parsing system will be happy.
	env := EnvsType()

	// we're actually going to ignore this flag entirely and parse the commandline
	// arguments early instead
	_ = cmd.PersistentFlags().String("env", env,
		"determines which set of configuration env to use. can either be 'dev' or 'test' or 'release'")
	setSourceAnnotation(cmd.PersistentFlags(), "env", FlagSource)
	...
	switch env {
	case DefaultsDev:
		return UseDevEnvOrDefaults(identify)
	case DefaultsRelease:
		return UseReleaseEnvOrDefaults(identify)
	case DefaultsTest:
		return UseTestEnvOrDefaults(identify)
	default:
		panic(fmt.Sprintf("unsupported defaults value %q", FindDefaultsParam()))
	}
}
