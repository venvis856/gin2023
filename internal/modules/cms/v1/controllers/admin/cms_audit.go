...
type CmsAuditController struct{}

func (*CmsAuditController) Items(c *gin.Context) {
	var param struct {
	...
	model := global.DB.Model(&models.CmsAudit{})
...
// 软删除
func (*CmsAuditController) Delete(c *gin.Context) {
	var param struct {
	...
	result := global.DB.Model(&models.CmsAudit{}).Where("id = ?", param.Id).Updates(map[string]interface{}{
		"status":      9,
