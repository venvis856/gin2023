...
func (*PermissionController) Create(c *gin.Context) {
	...
	if rs.Error != nil {
		global.Response.Error(c, errcode.ERROR_SERVER, rs.Error.Error())
		return
	}
...
func (*PermissionController) Delete(c *gin.Context) {
	...
	if permissionInfo == nil {
		global.Response.Error(c, errcode.ERROR_SERVER, "无该权限")
		return
	}
	// 删除用户权限
	...
	global.Response.Success(c,  "删除成功")
}
...
func (*PermissionController) GetMenuByUser(c *gin.Context) {
	...
	permissionList:=service.PermissionOperate().GetAllPermissionByUser(int64(userId),param.SiteId)
