...
func (a *identifyLogic) Info(param v1.InfoReq) (map[string]interface{}, error) {
	...
	global.DB.Model(&models2.Identify{}).Where("status != ?", 9).First(&identifyInfo, param.Id)
	// 先找到系统管理员角色 , 身份详情会把系统管理员的信息也带过去
	var roleInfo models2.Role
	global.DB.Model(&models2.Role{}).Where("status != 9 and type=1 and identify_id=?", param.Id).First(&roleInfo)
	if roleInfo.ID == 0 {
		return result, errors.New("该系统管理员异常")
	}
	// 根据用户角色表，找到管理员的user id
	var userIdsSlice []int64
	err := global.DB.Model(&models2.UserRole{}).Where("is_effective == 1 and identify_id = ? and role_id= ?", param.Id, roleInfo.ID).Pluck("id", &userIdsSlice).Error
	if err != nil {
		return result, errors.New(fmt.Sprintf("role get user id err :%v", err))
	}
	var adminUserList []models2.User
	global.DB.Model(&models2.User{}).Where("status != 9 and id in  ?", userIdsSlice).
		First(&adminUserList)
...
func (a *identifyLogic) Create(param v1.CreateReq) (int64, error) {
	// 校验如果新增的是系统，只能初始化一次
	if int(param.Type) == models2.IDENTIFY_TYPE_SYSTEM {
		var systemIdentifyInfo models2.Identify
		global.DB.Model(&models2.Identify{}).Where("type = ?", models2.IDENTIFY_TYPE_SYSTEM).First(&systemIdentifyInfo)
	...
	// 开始新增
	tx := global.DB.Begin()
	model := tx.Model(&models2.Identify{})

	// add identify
	identifyData := models2.Identify{
	...
	// 新增管理员
	userModel := tx.Model(&models2.User{})
	...
	// 新增 用户角色表
	userRoleData := models2.UserRole{
		IdentifyId:  identifyData.ID,
		UserId:      userData.ID,
		RoleId:      roleData.ID,
		IsEffective: consts.EFFECTIVE_YES,
	}
	result = tx.Model(&models2.UserRole{}).Create(&userRoleData)
	if result.Error != nil {
		tx.Rollback()
...
func (a *identifyLogic) Update(param v1.UpdateReq) (int64, error) {
	...
	if int(param.Type) == models2.IDENTIFY_TYPE_SYSTEM {
		var systemIdentifyInfo models2.Identify
		global.DB.Model(&models2.Identify{}).Where("type = ? and id !=? ", models2.IDENTIFY_TYPE_SYSTEM, param.Id).First(&systemIdentifyInfo)
		if systemIdentifyInfo.ID != 0 {
	...
	tx := global.DB.Begin()
	identifyModel := tx.Model(&models2.Identify{})
...
func (a *identifyLogic) Delete(param v1.DeleteReq) (int64, error) {
	var info models2.Identify
	global.DB.Model(&models2.Identify{}).Where("id = ?", param.Id).First(&info)
	if info.ID != 0 {
		return 0, errors.New("不存在的标识")
	}

	if int(info.Type) == models2.IDENTIFY_TYPE_SYSTEM {
		return 0, errors.New("系统类型不可删除")
	}

	result := global.DB.Model(&models2.Identify{}).Where("id = ?", param.Id).Updates(map[string]interface{}{
		"status":      9,
